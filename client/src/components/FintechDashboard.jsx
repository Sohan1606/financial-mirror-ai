...
import { calculateFinancialAnalysis, getCategoryChartData, getMonthlyChartData, getMonthlyIncomeExpenseData, generateInsights, calculateHealthScore, getHealthScoreColor } from '../utils/financialCalculations';
      ...
      {monthlyIncomeExpenseData.length > 0 && (
        <div className="bg-gradient-to-br from-slate-900/80 to-slate-800/50 backdrop-blur-xl border border-slate-800/50 rounded-3xl p-8 shadow-2xl">
          ...
          <ResponsiveContainer width="100%" height={400}>
            <ComposedChart data={monthlyIncomeExpenseData}>
              <CartesianGrid vertical={false} stroke="hsl(220 20% 15%)" strokeOpacity={0.5} />
              <XAxis 
                dataKey="month" 
                axisLine={false} 
              ...
              <Tooltip 
                ...
                formatter={(value, name) => [formatINR(value), name === 'savings' ? 'Net Savings' : name.charAt(0).toUpperCase() + name.slice(1)]}
              />
              <Legend 
          ...
          <div className="mt-6 pt-6 border-t border-slate-700/50 grid grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <div className="text-lg font-bold text-emerald-400">
                {formatINR(monthlyIncomeExpenseData.reduce((sum, d) => sum + d.income, 0))}
              </div>
            </div>
            <div className="text-center">
              <div className="text-sm text-slate-400 mb-1">Total Expenses</div>
              <div className="text-lg font-bold text-red-400">
                {formatINR(monthlyIncomeExpenseData.reduce((sum, d) => sum + d.expense, 0))}
              </div>
            </div>
            <div className="text-center">
              <div className="text-sm text-slate-400 mb-1">Net Savings</div>
              <div className="text-lg font-bold text-blue-400">
                {formatINR(monthlyIncomeExpenseData.reduce((sum, d) => sum + d.savings, 0))}
              </div>
            </div>
          </div>
        </div>
